...
const item = (kind: string, quantity: number, unitValue: number, unitCost = 0) => ({
  kind,
  quantity,
  unitValue,
  unitCost,
...
describe('Metas e vigência da versão do Barter', () => {
  const barters = [
    {
      status: 'approved',
      items: [item('grain', 100, 148.5), item('input', 10, 115, 90), item('input', 20, 18.9, 15)],
    },
    { status: 'approved', items: [item('grain', 50, 148.5), item('input', 5, 320, 250)] },
    // Não entram na conta: uma ainda pode ser negada, a outra já foi.
    { status: 'pending', items: [item('grain', 999, 148.5), item('input', 99, 115, 90)] },
    { status: 'denied', items: [item('grain', 999, 148.5), item('input', 99, 115, 90)] },
  ];

  it('o realizado conta só as permutas aprovadas', () => {
    expect(realizedFrom(barters)).toEqual({
      sales: 10 * 115 + 20 * 18.9 + 5 * 320, // 3128
      profit: 10 * 25 + 20 * 3.9 + 5 * 70, // 678
      sacks: 150,
      barters: 2,
    });
  });
  ...
  it('sem permuta aprovada, o realizado é zero em todas as unidades', () => {
    expect(realizedFrom([{ status: 'pending', items: [item('input', 1, 10, 5)] }])).toEqual({
      sales: 0,
      profit: 0,
      sacks: 0,
      barters: 0,
  ...
  it('meta não definida não vira barra na tela', () => {
    const goals = goalsOf(
      { targetSales: 5000, targetProfit: null, targetSacks: null, targetBarters: null },
  ...
  it('meta atingida acende o aviso, e a barra não passa de 100%', () => {
    const goals = goalsOf(
      { targetSales: 3000, targetProfit: null, targetSacks: 150, targetBarters: null },
      realizedFrom(barters),
    );
